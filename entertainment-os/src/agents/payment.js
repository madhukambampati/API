...
export function authorize({ amount, method, upiId, country }) {
  ...
  return {
    ref,
    method,
    label: method === 'upi' ? `UPI (${String(upiId).trim()})` : m.label,
    amount,
    currency: 'INR',
...
export function companyCard(amount) {
  return { ref: `CORP-${Date.now().toString(36).toUpperCase()}`, method: 'company-card', label: 'Company card (cost center)', amount, currency: 'INR', status: 'charged on approval', demo: true, at: new Date().toISOString() };
}
